...
export function buildPrompt(ctx: ContextPacket): string {
  ...
  if (taskLower.includes('gsd')) {
    lines.push('HINT: When executing a full GSD pipeline for phase N, the canonical sequence is shown below for REFERENCE. DO NOT auto-advance through these steps — issue ONE command per cycle and wait for the human / worker signal before moving to the next step. The numbered list is informational, not a checklist you should execute back-to-back:');
    ...
    lines.push('REMINDER: DO NOT auto-advance between the steps above. Wait for explicit human instruction or a clear worker signal before issuing each next GSD command.');
    lines.push('');
  }
